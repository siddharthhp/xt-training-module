...
class Menu extends Component {
  ...
  render() {
    const subcourse = this.props.subCourseList[0] ? this.props.subCourseList[0].subcourses[0] : '';
    const isLoggedIn = this.props.isLoggedIn;
    console.log('menu render isLoggedIn: ' + isLoggedIn);
      ...
      <div ref = {elem => this.elem = elem}  className ="col-md-3">
        <div className ="bs-sidebar hidden-print affix well" role="complementary">
          <ul className ="nav bs-sidenav">
            <li className={liClass} ><a href="#" data-subcourse={subcourse} onClick={this.handleAssessment}>Submit Assessments</a></li>
              {
                this.props.subCourseList.map((subcourse,index) => {
                  return (<li key={index} className ={index == 0 ? "main active" : ""} >
                      <a href={`#${subcourse.acf.contentid}`} 
                          dangerouslySetInnerHTML={{__html: sanitizeHtml(subcourse.title.rendered, {
