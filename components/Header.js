...
class Header extends Component {
  ...
  triggerSearch = async event => {
    ...
    const searchResult = await fetch(
      `${Config.apiUrl}/wp-json/wp/v2/courses?search=${
  ...
  nextClickHandler = async event => {
    ...
    if (courses[0].classList.contains('active')) {
      event.currentTarget.parentElement.previousElementSibling.classList.add(
        'disabled'
      );
      ...
      event.currentTarget.parentElement.previousElementSibling.classList.remove(
        'disabled'
      );
    }
  };
  ...
  selectCourse = (event, position) => {
    ...
    event.stopPropagation();
    var courses = this.elem.querySelectorAll('.course-list li:not(.dropdown)');
    var activeElement = this.elem.querySelectorAll(
      '.course-list li.active:not(.dropdown)'
    )[0];
    var currElement =
      courses[Array.prototype.indexOf.call(courses, activeElement)];
    var selectedElem =
      position == 'next'
    ...
    if (
      selectedElem.parentElement.closest('li') &&
      selectedElem.parentElement.closest('li').classList.contains('dropdown')
    ) {
      selectedElem.parentElement.closest('li').classList.add('active');
    } else if (
      this.elem.querySelectorAll('.course-list li.active.dropdown')[0]
    ) {
      this.elem
        .querySelectorAll('.course-list li.active.dropdown')[0]
  ...
  searchItemClickHandler = event => {
    ...
    for (var i = 0; i < courses.length; i++) {
      if (
        courses[i].getAttribute('data-courseid') ==
        event.currentTarget.getAttribute('data-courseid')
      ) {
        courses[i].click();
        setTimeout(() => {
                  ...
                  return (
                    <li
                      ...
                      className={
                        index == 0
                          ? 'active'
                          : course.subcourseList.length
                            ? 'dropdown'
                            : ''
                      }
                      onClick={this.courseItemClickHandler}
                    >
                      <a
                        href="."
                        className={
                          course.subcourseList.length ? 'dropdown-toggle' : ''
                        }
                      >
                        {course.description}
                        <b className="caret" />
                      ...
                      <ul className="dropdown-menu">
                        {course.subcourseList.map((subcourse, index) => {
                          return (
                            <li
                              key={index}
                              data-courseid={subcourse.id}
                              onClick={this.subCourseItemClickHandler}
                            >
              ...
              {this.state.searchResult.map((course, index) => {
                return (
                  <li
                    ...
                    key={index}
                    data-courseid={course.subcourses[0]}
                    data-contentid={course.acf.contentid}
                    onClick={this.searchItemClickHandler}
                  >
                    <a
                      href="."
                      dangerouslySetInnerHTML={{
                        __html: sanitizeHtml(course.title.rendered, {
